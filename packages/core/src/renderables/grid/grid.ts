...
export default class GridRenderable extends GridNodeRenderable {
    ...
    private focusLocalNeighbor(
        ...
        const currentFocusLocal = this.currentFocusLocal;
        const [x, y] = currentFocusLocal.coords;
    ...
    private handleGridKeys(ev: KeyEvent) {
        ...
        switch (ev.name) {
            case movement.left?.name:
                this.focusLocalNeighbor("left", [0, -1], ev);
                break;
            case movement.right?.name:
                this.focusLocalNeighbor("right", [0, 1], ev);
                break;
            case movement.up?.name:
                this.focusLocalNeighbor("up", [-1, 0], ev);
                break;
            case movement.down?.name:
                this.focusLocalNeighbor("down", [1, 0], ev);
                break;
        }
    ...
    override add(node: unknown, index?: number | undefined): number {
        if (node instanceof GridNodeRenderable) {
            ...
            if (this.getChildren().length === 0) {
                // NOTE: This trick is to mimic an `onMount` behavior. There might be a better way.

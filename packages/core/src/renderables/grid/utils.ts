...
import type { KeySettingsCondition } from "./types"
...
const possibleConditions: KeySettingsCondition[] = ["ctrl", "shift", "meta", "option"]
/** Check if a KeyEvent matches conditions 1:1 */
export const eventMatchesConditions = (ev: KeyEvent, conditions: KeySettingsCondition[] | undefined) =>
  possibleConditions.every((condition) =>
    ev[condition] ? conditions?.includes(condition) : !conditions?.includes(condition),
  )
